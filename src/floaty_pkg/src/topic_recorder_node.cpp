...
std::string data_topic; //, data_file_path;
...
{
    private:
        float x_pos_arr[72000];
        float y_pos_arr[72000];
        float z_pos_arr[72000];
        float x_rot_arr[72000];
        float y_rot_arr[72000];
        float z_rot_arr[72000];
        float w_rot_arr[72000];
    ...
    DataCollector() : x_pos(0), y_pos(0), z_pos(0), x_rot(0), y_rot(0), z_rot(0), w_rot(0), data_to_record(0), file_name("data.csv"), recording_done(false), counter(0), receiver_sensor_data(false), use_data(use_data_directly) { }  // , file_path(data_file_path)
...
{
    ros::init(argc, argv, "topic_recorder_node");
    ros::NodeHandle nh;

    nh.param<std::string>("/topic_recorder/data_topic",data_topic, "/Optitrack/Floaty/");
    // nh.param<std::string>("/topic_recorder/data_file_path",data_file_path, "/recorded_data.csv");

    nh.getParam("/topic_recorder/record_duration",record_duration);
    nh.getParam("/topic_recorder/use_data_directly",use_data_directly);
    ...
    ros::Subscriber sub_mocap = nh.subscribe(data_topic, 1000, &DataCollector::forceCallback, &data_collector_object);
